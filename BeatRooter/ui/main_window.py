...
from core.flipper_workspace_importer import FlipperWorkspaceImporter
from core.flipper_device_manager import FlipperDeviceManager
from core.flipper_serial_storage import FlipperSerialStorageMirror, FlipperSerialBusyError, FlipperSerialError

from ui.canvas_widget import CanvasWidget
from ui.flipper_explorer_dialog import FlipperExplorerDialog
...
class DigitalDetectiveBoard(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.canvas_widget.setObjectName("CanvasColumn")
    ...
    def import_flipper_workspace(self):
        ...
        if not root_paths and serial_ports:
            self.statusBar().showMessage(f'Flipper RPC detected ({serial_ports[0]}). Syncing remote storage...')
            try:
                with FlipperSerialStorageMirror(serial_ports[0]) as mirror:
                ...
                print(f"Flipper serial mirror error: {exc}")
            except Exception as exc:
                print(f"Unexpected Flipper RPC mirror error: {exc}")
        ...
        if rpc_without_filesystem:
            self.statusBar().showMessage(f'Flipper RPC detected ({serial_ports[0]}). Using selected storage path.')
        else:
    ...
    def import_selected_flipper_files(self, file_paths: list, root_path: str = "", drop_scene_pos: QPointF = None):
        ...
        for index, file_path in enumerate(valid_paths):
