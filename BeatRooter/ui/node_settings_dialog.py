...
class NodeSettingsDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        info = QLabel(
            "Disable core defaults and customize predefined templates with field_name + content (text only).\n"
        ...
        override_group = QGroupBox("Predefined Node Template Override")
        ...
        self.node_type_combo.setMaxVisibleItems(16)
        ...
        self.added_fields_scroll.setFixedHeight(156)
        ...
        self.removed_fields_list.setFixedHeight(140)
        ...
        root.addWidget(override_group, 1)
    ...
    def _update_adaptive_heights(self):
        ...
        if height >= 760:
            ...
            # Compact mode for smaller windows; keep both sections usable.
            added_height = max(96, min(156, int(height * 0.17)))
            removed_height = max(96, min(160, int(height * 0.20)))

        self.added_fields_scroll.setFixedHeight(added_height)
        self.removed_fields_list.setFixedHeight(removed_height)
    ...
    def _load_node_type_options(self):
        self._node_type_options = []
        for node_type in NodeFactory.list_predefined_node_types():
            category = NodeFactory.get_category_display_name(NodeFactory.find_node_category(node_type))
            display = f"{category} > {NodeFactory.get_node_name(node_type)} ({node_type})"
            self._node_type_options.append((display, node_type))
    ...
    def _filter_node_type_options(self):
        ...
        for display, node_type in self._node_type_options:
            if search and search not in display.lower() and search not in node_type.lower():
                continue
    ...
    def _load_current_override(self):
        ...
        if not node_type:
            ...
            self.status_label.setText("No predefined node types available.")
            return

        override = NodeFactory.get_predefined_node_template_override(node_type)
    ...
    def _apply_current_override(self, show_feedback: bool = False) -> bool:
        ...
        try:
            ...
            NodeFactory.set_predefined_node_template_override(node_type, added_fields, removed_fields)
    ...
    def _clear_current_override(self):
        ...
        if not node_type:
            return
        NodeFactory.clear_predefined_node_template_override(node_type)
        self._changes_applied = True
        self._load_current_override()
