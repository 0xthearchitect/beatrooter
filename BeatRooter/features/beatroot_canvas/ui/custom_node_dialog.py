...
class CustomNodeDialog(QDialog):
    def __init__(self, parent=None, default_category: str = None):
        super().__init__(parent)
        self.setWindowTitle("Create Custom Node")
        self.setMinimumWidth(520)
        self._field_rows = []
        self._default_category = NodeFactory.normalize_category(default_category)
        self._build_ui()
    ...
    def _build_ui(self):
        ...
        self.new_category_edit.setPlaceholderText("Optional. New category name (goes to user categories)")
        form.addRow("New Category:", self.new_category_edit)
    ...
    def _load_category_options(self):
        ...
        self.category_combo.addItem("Custom Nodes (Default)", NodeFactory.CUSTOM_CATEGORY)
        custom_categories = NodeFactory.list_user_custom_categories()

        known_categories = set(NodeFactory.CATEGORY_CATALOG.keys())
        for category in NodeFactory.get_node_type_categories().values():
            normalized = NodeFactory.normalize_category(category)
            if (
                normalized
                and normalized != NodeFactory.CUSTOM_CATEGORY
                and normalized not in custom_categories
            ):
                known_categories.add(normalized)

        for category in sorted(known_categories, key=lambda item: NodeFactory.get_category_display_name(item).lower()):
            self.category_combo.addItem(NodeFactory.get_category_display_name(category), category)

        for category, label in sorted(custom_categories.items(), key=lambda item: item[1].lower()):
            self.category_combo.addItem(f"{label} (User)", category)

        target_category = self._default_category or NodeFactory.CUSTOM_CATEGORY
        index = self.category_combo.findData(target_category)
    ...
    def get_template_payload(self):
        ...
        selected_category = self.category_combo.currentData() or NodeFactory.CUSTOM_CATEGORY
        new_category_name = self.new_category_edit.text().strip()
        category_key = selected_category
        category_label = ""

        if new_category_name:
            category_key = self._sanitize_field_key(new_category_name)
            if not category_key:
                QMessageBox.warning(self, "Invalid Category", "Please provide a valid category name.")
                return None
