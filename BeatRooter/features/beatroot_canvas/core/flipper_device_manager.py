...
class FlipperDeviceManager:
    ...
    SERIAL_PORT_HINTS = ("ttyacm", "ttyusb", "flipper", "usb serial", "usb modem", "virtual com", "cdc", "uart")
    ...
    def find_connected_flipper_ports(cls) -> list:
        system_name = platform.system().lower()
        ports = []
        ports.extend(cls._qt_serial_candidates())
    ...
    @classmethod
    def _qt_serial_candidates(cls) -> list:
        ...
        try:
            for port_info in QSerialPortInfo.availablePorts():
                port_name = (port_info.portName() or "").strip()
                system_location = (port_info.systemLocation() or port_name).strip()
                ...
                haystack = " ".join((port_name, system_location, description, manufacturer, serial_number)).lower()
                if not system_location:
                    continue

                if "flipper" in haystack:
                    strong_candidates.append(system_location)
                    continue

                if any(hint in haystack for hint in cls.SERIAL_PORT_HINTS):
                    weak_candidates.append(system_location)
                    continue

                if platform.system().lower() == "windows" and port_name.upper().startswith("COM"):
                    weak_candidates.append(system_location)
        except Exception:
            return []

        return strong_candidates + weak_candidates
            ...
            for path in Path("/dev").glob(Path(pattern).name):
                try:
                    if path.exists():
                        ...
                        elif any(token in path.name.lower() for token in ("ttyacm", "ttyusb")):
                            weak_candidates.append(str(path))
                except OSError:
                    continue

        if not candidates:
            # Last resort: expose likely USB serial ports so user can still proceed with RPC/manual path.
            candidates.extend(weak_candidates[:6])
    ...
    def _windows_serial_candidates(cls) -> list:
        ...
        for match in cls._iter_windows_com_matches(output):
            candidates.append(match)

        return candidates

    @classmethod
    def _iter_windows_com_matches(cls, text: str):
        seen = set()
        for match in re.finditer(r"\bCOM\d+\b", text or "", flags=re.IGNORECASE):
            port_name = match.group(0).upper()
