...
class NodeFactory:
    CATEGORY_TEMPLATES = {
        ...
        'crypto': {
            ...
            'crypto_asset': {
                ...
                'color': '#0ea5e9',
                'default_data': {
                    'asset_name': 'Primary Database Encryption',
                    'asset_type': 'data_at_rest',
            ...
            'key_management_event': {
                'name': 'Key Management Event',
                'color': '#2563eb',
                'default_data': {
                    'event_type': 'rotation',
                    'key_id': '',
    ...
    CATEGORY_CATALOG = {
        ...
        "crypto": {
            "name": "Crypto",
            "description": "Track encryption posture, key lifecycle, and certificate hygiene.",
            "color": "#0ea5e9",
            "icon": "CR",
            "project_types": ["blueteam"],
        },
        ...
        "flipper": {
            "name": "Flipper",
            "description": "Flipper Zero artifacts and telemetry.",
            "color": "#0284c7",
            "icon": "FL",
            "project_types": ["redteam", "blueteam", "soc_team"],
        },
    }
    ...
    DEFAULT_NODE_SYMBOLS = {
        ...
        "public_key": "[PUBK]",
        "crypto_asset": "[CRAST]",
        "key_management_event": "[KEYMG]",
        "certificate_record": "[CERT]",
        "crypto_finding": "[CRFND]",
    ...
    def get_category_display_name(cls, category: str) -> str:
        ...
        custom_categories = cls.list_user_custom_categories()
        if normalized in custom_categories:
            return custom_categories[normalized]
        return (normalized or "general").replace("_", " ").title()

    @classmethod
    def _normalize_custom_template_category(cls, category: str = None) -> str:
        normalized = cls.normalize_category(category)
        if not normalized:
    ...
    @classmethod
    def is_custom_node_type(cls, node_type: str) -> bool:
        return node_type in cls.CUSTOM_NODE_TEMPLATES

    @classmethod
    def list_user_custom_categories(cls) -> dict:
        categories = {}
        for template in cls.CUSTOM_NODE_TEMPLATES.values():
            if not isinstance(template, dict):
                continue
            category_key = cls._normalize_custom_template_category(template.get("category"))
            if category_key in cls.CATEGORY_CATALOG or category_key == cls.CUSTOM_CATEGORY:
                continue
            category_label = (template.get("category_label") or "").strip()
    ...
    @classmethod
    def _is_known_node_type(cls, node_type: str) -> bool:
        if cls._is_predefined_node_type(node_type):
            return True
        return node_type in cls.CUSTOM_NODE_TEMPLATES
    ...
    @classmethod
    def get_node_template_override(cls, node_type: str) -> dict:
        override = cls.NODE_TEMPLATE_OVERRIDES.get(node_type, {})
    ...
    @classmethod
    def set_node_template_override(cls, node_type: str, added_fields: dict = None, removed_fields: list = None):
        if not cls._is_known_node_type(node_type):
            raise ValueError(f"'{node_type}' is not a known node type")
        ...
        if not clean_added_fields and not clean_removed_fields:
            cls.NODE_TEMPLATE_OVERRIDES.pop(node_type, None)
            return

        cls.NODE_TEMPLATE_OVERRIDES[node_type] = {
            "added_fields": clean_added_fields,
            "removed_fields": clean_removed_fields,
        }

    @classmethod
    def clear_node_template_override(cls, node_type: str):
        cls.NODE_TEMPLATE_OVERRIDES.pop(node_type, None)

    @classmethod
    def get_predefined_node_template_override(cls, node_type: str) -> dict:
        return cls.get_node_template_override(node_type)
    ...
    def set_predefined_node_template_override(cls, node_type: str, added_fields: dict = None, removed_fields: list = None):
        ...
        cls.set_node_template_override(node_type=node_type, added_fields=added_fields, removed_fields=removed_fields)

    @classmethod
    def clear_predefined_node_template_override(cls, node_type: str):
        cls.clear_node_template_override(node_type)

    @classmethod
    def export_node_template_settings(cls) -> dict:
        return {
            "core_field_settings": cls.get_core_field_settings(),
            "node_overrides": copy.deepcopy(cls.NODE_TEMPLATE_OVERRIDES),
        }

    @classmethod
    def reset_node_template_settings(cls):
        cls.reset_core_field_settings()
        cls.NODE_TEMPLATE_OVERRIDES = {}
    ...
    def load_node_template_settings(cls, settings_payload: dict):
        ...
        overrides = settings_payload.get("node_overrides")
        # Backward compatibility for investigation files saved before node_overrides.
        ...
        for node_type, payload in overrides.items():
            if not isinstance(payload, dict):
                continue
            try:
                cls.set_node_template_override(
                    node_type=node_type,
    ...
    def register_custom_node_template(
        ...
        category_key = cls._normalize_custom_template_category(category)
        category_name = (category_label or "").strip()
        if category_key in cls.CATEGORY_CATALOG or category_key == cls.CUSTOM_CATEGORY:
            category_name = ""
        ...
        if category_name:
            template["category_label"] = category_name
    ...
    def load_custom_node_templates(cls, templates: dict):
        ...
        for node_type, template in templates.items():
            ...
            category = template.get("category", cls.CUSTOM_CATEGORY)
            category_label = template.get("category_label", "")
            if str(node_type).startswith("flipper_"):
                category = "flipper"
                category_label = ""
            try:
    ...
    def get_available_node_types(cls, category: str = None) -> list:
        ...
        if normalized_category == cls.CUSTOM_CATEGORY:
            typed = []
            for node_type, template in cls.CUSTOM_NODE_TEMPLATES.items():
                template_category = cls._normalize_custom_template_category(template.get("category"))
                if template_category == cls.CUSTOM_CATEGORY:
                    typed.append(node_type)
            return sorted(typed, key=lambda item: cls.get_node_name(item).lower())

        typed = []

        if normalized_category and normalized_category in cls.CATEGORY_TEMPLATES:
            typed.extend(cls.CATEGORY_TEMPLATES[normalized_category].keys())
        ...
        if normalized_category:
            ...
            for node_type, template in cls.CUSTOM_NODE_TEMPLATES.items():
                ...
                if template_category == normalized_category:
                    typed.append(node_type)
            return sorted(set(typed), key=lambda item: cls.get_node_name(item).lower())
    ...
    def get_node_type_categories(cls) -> dict:
        ...
        for node_type, template in cls.CUSTOM_NODE_TEMPLATES.items():
            categories[node_type] = cls._normalize_custom_template_category(template.get("category"))
    ...
    def find_node_category(cls, node_type: str):
        if node_type in cls.CUSTOM_NODE_TEMPLATES:
            template = cls.CUSTOM_NODE_TEMPLATES.get(node_type, {})
            return cls._normalize_custom_template_category(template.get("category"))
