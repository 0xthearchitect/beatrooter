...
class FlipperSerialStorageMirror:
    ...
    def open(self):
        if QSerialPort is not None and QIODeviceBase is not None:
            self._open_qt_serial()
            return

        if termios is None:
            raise FlipperSerialError("No supported serial backend available on this platform")

        self._open_posix_serial()
    ...
    def _open_qt_serial(self):
        port_name = self._normalize_port_name(self.port_path)
        serial_port = QSerialPort()
        serial_port.setPortName(port_name)
        serial_port.setBaudRate(self.DEFAULT_BAUD)
        ...
        if not serial_port.open(QIODeviceBase.OpenModeFlag.ReadWrite):
            error_text = serial_port.errorString() or f"Failed to open serial port {port_name}"
            lowered = error_text.lower()
            if any(token in lowered for token in ("permission", "denied", "busy", "resource", "in use")):
                raise FlipperSerialBusyError(error_text)
            raise FlipperSerialError(error_text)
    ...
    def _write_raw(self, payload: bytes):
        if self.serial_port is not None:
            ...
            if written == -1:
                raise FlipperSerialError(self.serial_port.errorString() or "Serial write failed")
            self.serial_port.waitForBytesWritten(int(self.READ_TIMEOUT * 1000))
            return
        if self.fd is None:
    ...
    def _read_until_quiet(self, timeout: float) -> bytes:
        if self.serial_port is not None:
            ...
            while time.time() - start < timeout:
                if self.serial_port.waitForReadyRead(30):
    ...
    def _normalize_port_name(self, value: str) -> str:
