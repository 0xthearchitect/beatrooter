...
install_repo() {
    ...
    echo -e "${GREEN}[✓] Use: whois dominio.com${NC}"
}
...
remove_tool() {
    ...
    case $PM in
        apt)
            sudo apt remove -y whois
            sudo apt autoremove -y
            ;;
        dnf)
            sudo dnf remove -y whois
            ;;
        pacman)
            sudo pacman -R --noconfirm whois
            ;;
        zypper)
            sudo zypper remove -y whois
            ;;
    esac
